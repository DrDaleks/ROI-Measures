	...
	private static synchronized WritableSheet getOrCreateSheet(Sequence sequence)
	{
		...
		if (sheet == null)
		{
			...
			try
			{
				...
				{
					sheet.addCell(new Label(5 + 4 * c + 1, 0, "min. (ch " + c + ")"));
					sheet.addCell(new Label(5 + 4 * c + 2, 0, "max. (ch " + c + ")"));
	...
	private void update(Sequence sequence, ROI2D roi)
	{
		try
		{
			...
			for (int c = 0; c < sum.length; c++)
			{
				...
				sheet.addCell(new Number(5 + 4 * c + 2, row, max[c]));
				sheet.addCell(new Number(5 + 4 * c + 3, row, sum[c] / cpt[c]));
				sheet.addCell(new Number(5 + 4 * c + 4, row, sum[c]));
			}
		}
		catch (RowsExceededException e)
		{
