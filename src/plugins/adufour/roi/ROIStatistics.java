...
public class ROIStatistics extends Plugin implements ROIBlock
{
	...
	public void run()
	{
		...
		Sequence sequence = seq.getValue();
		int sizeC = sequence == null ? -1 : sequence.getSizeC();
		...
		for (ROI roi : rois)
		{
			...
			if (sequence != null)
			{
				sheetName = FileUtil.getFileName(sequence.getFilename());
			...
			if (header == null)
			{
				...
				header.getCell(COL_NAME).setCellStyle(style);
